...
def scrape_timetables(entry):
    try:
        ...
        with open(final_name, 'wb+') as f:
            f.write(r.content)
        fin_split = final_name.split('_')[0] #a little kluge but we have to preserve the file name with the space instead of the backslash to test if it exists, since python insistts on the space version instead of the slash version, while bash (and thus os.system) insists on backslash version
